...
Implement the /div command handler in src/handlers/div.ts. The handler should:
- Parse two numeric operands from the command arguments
- Compute the quotient
- Format the result using the formatNumber utility
- Handle invalid inputs with a usage hint
- Specifically check for division by zero and provide a friendly error message
- Write a BotSpec test in tests/specs/div.json
- Add the command to tests/commands.json
