# fix-5c82e247a75952de — E1T2: Imports formatNumber from toolkit instead of E1T5 utility

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MATHB

`src/handlers/E1T2.ts:3` imports `formatNumber` from `../toolkit/index.js`. The toolkit's `formatNumber` (`src/toolkit/format.ts`) uses `Intl.NumberFormat` with locale formatting (inserts commas: `1,000`) and 20 max fraction digits, instead of the E1T5 spec's requirement of 10 decimal places with trailing-zero trimming and no locale formatting. The E1T2 task spec requires using the E1T5 formatNumber utility.

**Fix:** Change line 3 to `import { formatNumber } from "../util/formatNumber.js";`.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-5c82e247a75952de.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-5c82e247a75952de.json` (and any new command `tests/commands/fix-5c82e247a75952de.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-5c82e247a75952de.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-5c82e247a75952de.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
