# fix-33d8403106737c1d — E1T4: Imports formatNumber from toolkit instead of E1T5 utility

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MATHB

`src/handlers/E1T4.ts:3` imports `formatNumber` from `../toolkit/index.js`. Same root cause as E1T2: the toolkit's `formatNumber` uses locale-aware `Intl.NumberFormat` (thousands separators, 20 fraction digits) instead of the E1T5 spec's `toFixed(10)` with trailing-zero trimming. The E1T4 task spec requires using the E1T5 formatNumber utility.

**Fix:** Change line 3 to `import { formatNumber } from "../util/formatNumber.js";`.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-33d8403106737c1d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-33d8403106737c1d.json` (and any new command `tests/commands/fix-33d8403106737c1d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-33d8403106737c1d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-33d8403106737c1d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
