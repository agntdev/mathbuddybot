# fix-d6350dae5cc7aafe — E1T1: Uses local formatNumber instead of E1T5 utility

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MATHB

`src/handlers/E1T1.ts:4-6` defines its own `formatNumber` function using `toLocaleString("en-US")`. The E1T1 task spec explicitly requires: "Format the result using the formatNumber utility" (the shared E1T5 utility at `src/util/formatNumber.ts`). This local function does not trim trailing zeros or cap at 10 decimal places, producing inconsistent output with the other arithmetic commands. For example, `0.1 + 0.2` would display as `"0.30000000000000004"` instead of the spec-correct `"0.3"`.

**Fix:** Replace the local `formatNumber` with `import { formatNumber } from "../util/formatNumber.js";` and remove lines 4-6.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-d6350dae5cc7aafe.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-d6350dae5cc7aafe.json` (and any new command `tests/commands/fix-d6350dae5cc7aafe.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-d6350dae5cc7aafe.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-d6350dae5cc7aafe.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
